...
class Claude(Cog):
    ...
    async def ClaudeCommand(
        ...
        Result = await Client.chat.completions.create(
            model="noxus/sonnet-3.5",
            messages=[{"role": "user", "content": prompt}],
            web_search=web_search,
        )
        Content = Result.text
        if not Content:
            await Interaction.followup.send(content="claude returned no response")
            return
        ...
        View.add_item(
            Container(
                ...
                TextDisplay(
                    content=f"Sonnet 3.5 • time taken {Elapsed} • invoked by {UserMention}"
                ),
